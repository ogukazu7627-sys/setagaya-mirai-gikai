-- Make completed private public-comment sessions permanently non-public while
-- keeping the existing opt-in review and publication flow unchanged.
...
begin
  ...
  -- Historical completions never acquire a receipt or change publication status.
...
create or replace function public.prevent_completed_private_comment_publication()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  if old.completed_at is not null
    and old.publication_status = 'private'
    and new.publication_status is distinct from 'private' then
    raise exception 'completed_private_public_comment_is_immutable';
  end if;
...
drop trigger if exists prevent_completed_private_comment_publication
  on public.public_comment_sessions;

create trigger prevent_completed_private_comment_publication
before update of publication_status on public.public_comment_sessions
for each row
execute function public.prevent_completed_private_comment_publication();

revoke all on function public.prevent_completed_private_comment_publication() from public;
revoke all on function public.prevent_completed_private_comment_publication() from anon;
revoke all on function public.prevent_completed_private_comment_publication() from authenticated;

...
> 初期公開では Google ログインは使わず、メールアドレス + パスワードで開始する。
> Google ログインを有効化する場合は、`supabase/config.toml` の
> `[auth.external.google].enabled` を `true` に戻すPRを別途作成する。
...
| `SUPABASE_AUTH_REDIRECT_URL` | `https://<admin-domain>/api/auth/callback` |

`SUPABASE_AUTH_REDIRECT_URL` は `supabase/config.toml` の `additional_redirect_urls` で `env()` 経由で参照される。

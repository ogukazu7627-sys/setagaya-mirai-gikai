export function formatBudgetQuestionCouncilorLabel(
  displayName: string
): string {
...
export function buildBudgetQuestionOverview(input: {
  councilorDisplayName: string;
  partyOrGroup: string | null;
  questionName: string;
}): string {
  const councilorLabel = formatBudgetQuestionCouncilorLabel(

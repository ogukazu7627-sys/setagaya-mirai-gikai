...
const GITHUB_ACTIONS_OIDC_AUDIENCE =
  "https://civictech-setagaya.org/api/cron/councilor-x-posts";
...
const GITHUB_WORKFLOW_REF = `${GITHUB_REPOSITORY}/.github/workflows/councilor_x_posts.yml@${GITHUB_MAIN_REF}`;
...
type CouncilorXPostSyncAuthDependencies = {
  verifyGithubActionsToken?: (token: string) => Promise<boolean>;
};

export async function isCouncilorXPostSyncRequestAuthorized(
  request: Request,
  dependencies: CouncilorXPostSyncAuthDependencies = {}
): Promise<boolean> {
  ...
  try {
    return await (
      ...
      verifyCouncilorXPostSyncGithubActionsToken
    )(token);
  } catch {
    return false;
  }
...
export function hasExpectedCouncilorXPostSyncGithubClaims(
  payload: JWTPayload
): boolean {
...
async function verifyCouncilorXPostSyncGithubActionsToken(
  token: string
): Promise<boolean> {
  ...
  return hasExpectedCouncilorXPostSyncGithubClaims(payload);
}

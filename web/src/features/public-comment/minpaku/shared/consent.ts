export const PUBLIC_COMMENT_CONSENT_VERSION = "2026-09-18-late-google-auth-v1";
export const PUBLIC_COMMENT_RECEIPT_LABEL =
  "終了後に、今回のインタビューとコメントの控えをメールで受け取る";
export const PUBLIC_COMMENT_EVENT_INVITATION_CONSENT_VERSION =
  "2026-09-20-event-invitation-v1";
export const PUBLIC_COMMENT_EVENT_INVITATION_LABEL =
  "この活動に関するイベント案内をメールで受け取る";
export const PUBLIC_COMMENT_EMAIL_NOTICE =
  "「確認して完了」を押した後、Googleログインのメールアドレスに、今回のAIとの会話全文と確認済みコメントの控えを送ります。イベント案内を希望した場合は、今回の活動に関する案内を別メールで送ります。どちらも任意で、最終文章を表示するためのGoogleログイン画面と下書き確認画面で変更できます。イベント案内に個別の回答内容は含めません。";
export const PUBLIC_COMMENT_AUTH_RETURN_KEY = "minpaku-interview-auth-return";
export const PUBLIC_COMMENT_AUTH_RECEIPT_KEY = "minpaku-interview-auth-receipt";

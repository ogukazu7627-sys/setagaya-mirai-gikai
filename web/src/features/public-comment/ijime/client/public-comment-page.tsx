...
const AUTH_RETURN_KEY = "ijime-interview-auth-return";
...
function formatDeadline() {
  return new Intl.DateTimeFormat("ja-JP", {
    ...
    day: "numeric",
    timeZone: "Asia/Tokyo",
  }).format(new Date(IJIME_SUBMISSION_DEADLINE));
}
  ...
  return (
    <div className="flex w-full max-w-[370px] flex-col items-center gap-3">
      ...
      <p className="text-xs text-mirai-text-secondary">
        {IJIME_LESSONS.length}章・各1問 / 目安{IJIME_LEARNING_ESTIMATED_TIME}
      </p>
      <Button
        type="button"
        variant="link"
    ...
    <div className="flex flex-col gap-8 pb-8">
      ...
      <div className="flex flex-col items-center gap-8 px-4">
        <div className="flex flex-col items-center gap-5 text-center">
          <div className="inline-flex items-center rounded-2xl bg-primary px-6 py-1 text-[13px] font-medium text-white">
            子どもから大人まで、区への意見を考えている方へ
          </div>
          ...
          <StartActions onStart={onStart} onLearn={onLearn} />
        </div>
            ...
            <div className="space-y-3 text-[13px] leading-6 text-black">
              ...
              <p>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="mx-1 font-bold text-primary underline underline-offset-4"
                >
                  「せたホッと」
                </a>
        ...
        <IntroSection title="確認した資料">
          ...
          <ul className="mt-4 space-y-3">
            {IJIME_SOURCES.map((source) => (
              <li key={source.id}>
                <a
                  href={source.url}
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-start gap-2 text-sm font-bold leading-6 text-primary underline-offset-4 hover:underline"
                >
  ...
  return (
    <div className="min-h-[calc(100dvh-var(--app-header-layout-offset))] bg-mirai-light-gradient px-4 py-8">
      <section className="mx-auto w-full max-w-[560px] rounded-2xl bg-white p-6">
        <p className="text-sm font-bold text-primary">インタビュー完了</p>
  ...
  return (
    <div className="min-h-[calc(100dvh-var(--app-header-layout-offset))] bg-mirai-light-gradient px-4 py-8">
      <section className="mx-auto w-full max-w-[720px] rounded-2xl bg-white p-6">
        ...
        <label
          htmlFor="ijime-public-comment-draft"
          className="mt-7 block text-sm font-bold"
        >
          提出用に編集する本文
        </label>
        <textarea
          id="ijime-public-comment-draft"
        ...
        <div className="mt-7 flex flex-col gap-3 sm:flex-row">
          ...
          <a
            href={IJIME_OFFICIAL_SUBMISSION_URL}
            target="_blank"
            rel="noreferrer"
            className={OUTLINE_BUTTON_CLASS}
          >
...
export function PublicCommentIjimePage() {
  ...
  const [sources, setSources] = useState<readonly IjimeSource[]>(IJIME_SOURCES);
  ...
  const signIn = async () => {
    ...
    await auth.signInWithGoogle(`${routes.publicCommentIjime()}?auth_return=1`);
  };
  ...
  const startSession = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch("/api/public-comment/ijime/session", {
        method: "POST",
  ...
  const sendAnswer = useCallback(
    async (value = answer) => {
      ...
      try {
        const response = await fetch("/api/public-comment/ijime/chat", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const generateDraft = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch("/api/public-comment/ijime/draft", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setDraft(data.draft);
      setSources(data.sources ?? IJIME_SOURCES);
      setView("review");
    } catch (caught) {
      setError(
        caught instanceof Error
  ...
  const complete = useCallback(async () => {
    ...
    try {
      if (!completionPending) {
        const updateResponse = await fetch("/api/public-comment/ijime/draft", {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
      ...
      const response = await fetch("/api/public-comment/ijime/complete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          sessionId,
  ...
  }, [busy, completionPending, draft, sessionId]);
  ...
  return (
    <div ref={containerRef}>
      ...
      {view === "complete" && (
        <CompletePage copied={copied} onCopy={() => void copyDraft()} />
      )}

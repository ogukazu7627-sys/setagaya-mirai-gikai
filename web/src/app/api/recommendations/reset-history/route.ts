import { resetRecommendationHistory } from "@/features/recommendations/server/repositories/recommendation-repository";
...
export async function POST(request: Request) {
  try {
    ...
    const profile = await resetRecommendationHistory(input.installationId);
    if (!profile) {
      throw new RecommendationProfileNotFoundError();
    }
    return jsonResponse({ preferenceVersion: profile.preference_version }, 200);
  } catch (error) {

...
import { GENDER_EQUALITY_QUESTIONS } from "@/features/public-comment/gender-equality/shared/campaign";
...
const request = (content = "私の意見") =>
  new Request("http://localhost/api/public-comment/gender-equality/chat", {
    method: "POST",
    body: JSON.stringify({ sessionId: "session-1", content }),
  });

describe("POST /api/public-comment/gender-equality/chat", () => {
  beforeEach(() => {
    vi.resetAllMocks();
    ...
    mocks.generate.mockResolvedValue({
      ...
      question_id: GENDER_EQUALITY_QUESTIONS[1].id,
      topic_title: GENDER_EQUALITY_QUESTIONS[1].topic,
      quick_replies: [],
  ...
  it("通常の政策意見は保存して次の質問へ進む", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      ...
      nextStage: "interview",
      topicTitle: GENDER_EQUALITY_QUESTIONS[1].topic,
    });
  });

  it("最終質問中でも危険が示されたら下書きへ進めず、安全案内を優先する", async () => {
    mocks.findMessages.mockResolvedValue(
      GENDER_EQUALITY_QUESTIONS.slice(0, -1).map((question, index) => ({
        id: `user-${index}`,
        role: "user",
        content: `回答${index + 1}`,
    ...
    mocks.generate.mockResolvedValue({
      text: "今すぐ危険がある場合は110へ、救急が必要なら119へ連絡してください。",
      question_id: null,
      topic_title: null,
      quick_replies: [],
      next_stage: "interview",
    ...
    mocks.appendMessage.mockReset().mockResolvedValueOnce({
      ...
      role: "assistant",
      content:
        "今すぐ危険がある場合は110へ、救急が必要なら119へ連絡してください。",
      question_id: null,
    });
  ...
  it("現在の危険を示す入力は保存せず、次の質問や選択肢を重ねない", async () => {
    mocks.generate.mockResolvedValue({
      text: "今すぐ危険がある場合は安全な場所へ移動し、110へ連絡してください。",
      question_id: null,
      topic_title: null,
      quick_replies: [],
      next_stage: "interview",
    ...
    mocks.appendMessage.mockReset().mockResolvedValueOnce({
      ...
      role: "assistant",
      content:
        "今すぐ危険がある場合は安全な場所へ移動し、110へ連絡してください。",
      question_id: null,
    });

    const response = await POST(request("現在、暴力から逃げている状況です"));

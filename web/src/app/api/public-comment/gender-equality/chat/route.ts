...
import { generateInterviewResponse } from "@/features/public-comment/gender-equality/server/ai";
import {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (previousAnswerCount >= GENDER_EQUALITY_QUESTIONS.length)
      return NextResponse.json(
    ...
    const isComplete = userAnswerCount >= GENDER_EQUALITY_QUESTIONS.length;
    const nextQuestion = isComplete
      ? null
      : GENDER_EQUALITY_QUESTIONS[userAnswerCount];
    ...
    if (!safetyRedirect) {
      await appendMessage({
        ...
        stage: "interview",
        questionId:
          GENDER_EQUALITY_QUESTIONS[Math.max(0, userAnswerCount - 1)]?.id ??
          null,
        content,
      });
    }
    ...
    console.error("GenderEquality public comment chat error");

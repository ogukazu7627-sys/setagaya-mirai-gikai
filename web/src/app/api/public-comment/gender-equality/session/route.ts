...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (messages.length === 0) {
      const firstQuestion = GENDER_EQUALITY_QUESTIONS[0];
      await appendMessage({
    ...
    if (answerCount >= GENDER_EQUALITY_QUESTIONS.length) {
      return NextResponse.json({
        sessionId: session.id,
        messages,
        quickReplies: [],
    ...
    const currentQuestion =
      GENDER_EQUALITY_QUESTIONS.find(
        (question) => question.id === lastAssistantQuestionId
      ) ?? GENDER_EQUALITY_QUESTIONS[0];
    ...
    console.error("GenderEquality public comment session error");
    return NextResponse.json(

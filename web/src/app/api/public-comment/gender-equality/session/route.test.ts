...
const request = () =>
  new Request("http://localhost/api/public-comment/gender-equality/session", {
    method: "POST",
    body: JSON.stringify({
      consented: true,
      receiptOptIn: false,
...
describe("POST /api/public-comment/gender-equality/session", () => {
  beforeEach(() => {
    vi.resetAllMocks();
    mocks.getUser.mockResolvedValue({ id: "google-user" });
    mocks.findCampaign.mockResolvedValue({
      id: "campaign-gender-equality",
      status: "published",
    });
    ...
    mocks.findMessages.mockResolvedValue([
      {
        id: "message-1",
        role: "assistant",
        content: GENDER_EQUALITY_QUESTIONS[0].question,
        question_id: GENDER_EQUALITY_QUESTIONS[0].id,
      },
    ]);
  });
  ...
  it("再開時もメール控えなしの同意を本人のセッションへ保存する", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      sessionId: "session-1",
      nextStage: "interview",
      quickReplies: GENDER_EQUALITY_QUESTIONS[0].quickReplies,
    });
  });

  it("7問の回答後は下書き作成から再開する", async () => {
    mocks.findMessages.mockResolvedValue(
      GENDER_EQUALITY_QUESTIONS.flatMap((question, index) => [
        {
          id: `assistant-${index}`,
          role: "assistant",

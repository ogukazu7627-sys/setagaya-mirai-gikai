  ...
  try {
    ...
    if (
      !canCreateInterviewDraft(
        ...
        DEMENTIA_HOPE_PLAN_QUESTIONS.length
      )
    )
      return NextResponse.json(
    ...
    const draft = await upsertDraft({
      sessionId,
      userId: user.id,
      targetOrdinances: [DEMENTIA_HOPE_PLAN_PLAN],
      aiBody: output.body,
      finalBody: output.body,
      sourceRefs: DEMENTIA_HOPE_PLAN_SOURCES.map(({ id, title, url }) => ({
        id,
        title,
        url,
      })),
    ...
    return NextResponse.json({ draft, sources: DEMENTIA_HOPE_PLAN_SOURCES });
  } catch (error) {
    ...
    console.error("DementiaHopePlan public comment draft generation error");
    return NextResponse.json(
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    console.error("DementiaHopePlan public comment draft update error");
    return NextResponse.json(

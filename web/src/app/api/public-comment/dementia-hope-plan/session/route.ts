...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (messages.length === 0) {
      const firstQuestion = DEMENTIA_HOPE_PLAN_QUESTIONS[0];
      await appendMessage({
    ...
    if (answerCount >= DEMENTIA_HOPE_PLAN_QUESTIONS.length) {
      return NextResponse.json({
        sessionId: session.id,
        messages,
        quickReplies: [],
    ...
    const currentQuestion =
      ...
      ) ?? DEMENTIA_HOPE_PLAN_QUESTIONS[0];
    ...
    console.error("DementiaHopePlan public comment session error");
    return NextResponse.json(

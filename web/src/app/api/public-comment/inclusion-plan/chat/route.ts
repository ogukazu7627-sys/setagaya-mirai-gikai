...
import { generateInterviewResponse } from "@/features/public-comment/inclusion-plan/server/ai";
import {
...
export async function POST(request: Request) {
  ...
  try {
    const campaign = await findCampaign(INCLUSION_PLAN_CAMPAIGN_SLUG);
    if (!campaign)
      return NextResponse.json(
    ...
    if (previousAnswerCount >= INCLUSION_PLAN_QUESTIONS.length)
      return NextResponse.json(
    ...
    const isComplete = userAnswerCount >= INCLUSION_PLAN_QUESTIONS.length;
    const nextQuestion = isComplete
      ? null
      : INCLUSION_PLAN_QUESTIONS[userAnswerCount];
    ...
    await appendMessage({
      ...
      stage: "interview",
      questionId:
        INCLUSION_PLAN_QUESTIONS[Math.max(0, userAnswerCount - 1)]?.id ?? null,
      content,
    });
    ...
    console.error("InclusionPlan public comment chat error");

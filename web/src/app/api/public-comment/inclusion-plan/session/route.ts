...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (messages.length === 0) {
      const firstQuestion = INCLUSION_PLAN_QUESTIONS[0];
      await appendMessage({
    ...
    if (answerCount >= INCLUSION_PLAN_QUESTIONS.length) {
      return NextResponse.json({
        sessionId: session.id,
        messages,
        quickReplies: [],
    ...
    const currentQuestion =
      INCLUSION_PLAN_QUESTIONS.find(
        (question) => question.id === lastAssistantQuestionId
      ) ?? INCLUSION_PLAN_QUESTIONS[0];
    ...
    console.error("InclusionPlan public comment session error");
    return NextResponse.json(

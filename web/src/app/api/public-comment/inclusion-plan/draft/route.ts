  ...
  try {
    ...
    if (
      !canCreateInterviewDraft(
        ...
        INCLUSION_PLAN_QUESTIONS.length
      )
    )
      return NextResponse.json(
    ...
    const draft = await upsertDraft({
      sessionId,
      userId: user.id,
      targetOrdinances: [INCLUSION_PLAN_PLAN],
      aiBody: output.body,
      finalBody: output.body,
      sourceRefs: INCLUSION_PLAN_SOURCES.map(({ id, title, url }) => ({
        id,
        title,
        url,
      })),
    ...
    return NextResponse.json({ draft, sources: INCLUSION_PLAN_SOURCES });
  } catch (error) {
    ...
    console.error("InclusionPlan public comment draft generation error");
    return NextResponse.json(
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    console.error("InclusionPlan public comment draft update error");
    return NextResponse.json(

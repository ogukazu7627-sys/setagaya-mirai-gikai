  ...
  try {
    ...
    if (
      !canCreateInterviewDraft(
        ...
        TRAFFIC_SAFETY_PLAN_QUESTIONS.length
      )
    )
      return NextResponse.json(
    ...
    const draft = await upsertDraft({
      sessionId,
      userId: user.id,
      targetOrdinances: [TRAFFIC_SAFETY_PLAN_PLAN],
      aiBody: output.body,
      finalBody: output.body,
      sourceRefs: TRAFFIC_SAFETY_PLAN_SOURCES.map(({ id, title, url }) => ({
        id,
        title,
        url,
      })),
    ...
    return NextResponse.json({ draft, sources: TRAFFIC_SAFETY_PLAN_SOURCES });
  } catch (error) {
    ...
    console.error("TrafficSafetyPlan public comment draft generation error");
    return NextResponse.json(
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    console.error("TrafficSafetyPlan public comment draft update error");
    return NextResponse.json(

...
import { TRAFFIC_SAFETY_PLAN_QUESTIONS } from "@/features/public-comment/traffic-safety-plan/shared/campaign";
...
const request = () =>
  new Request(
    "http://localhost/api/public-comment/traffic-safety-plan/session",
    {
      method: "POST",
      body: JSON.stringify({
        consented: true,
...
describe("POST /api/public-comment/traffic-safety-plan/session", () => {
  beforeEach(() => {
    vi.resetAllMocks();
    mocks.getUser.mockResolvedValue({ id: "google-user" });
    mocks.findCampaign.mockResolvedValue({
      id: "campaign-traffic-safety-plan",
      status: "published",
    });
    ...
    mocks.findMessages.mockResolvedValue([
      {
        id: "message-1",
        role: "assistant",
        content: TRAFFIC_SAFETY_PLAN_QUESTIONS[0].question,
        question_id: TRAFFIC_SAFETY_PLAN_QUESTIONS[0].id,
      },
    ]);
  });
  ...
  it("再開時もメール控えなしの同意を本人のセッションへ保存する", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      sessionId: "session-1",
      nextStage: "interview",
      quickReplies: TRAFFIC_SAFETY_PLAN_QUESTIONS[0].quickReplies,
    });
  });

  it("7問の回答後は下書き作成から再開する", async () => {
    mocks.findMessages.mockResolvedValue(
      TRAFFIC_SAFETY_PLAN_QUESTIONS.flatMap((question, index) => [
        {
          id: `assistant-${index}`,
          role: "assistant",

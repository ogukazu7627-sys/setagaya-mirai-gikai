...
const request = () =>
  new Request("http://localhost/api/public-comment/traffic-safety-plan/chat", {
    method: "POST",
    body: JSON.stringify({ sessionId: "session-1", content: "私の意見" }),
  });

describe("POST /api/public-comment/traffic-safety-plan/chat", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  ...
  it("最終回答もAIを通し、安全案内を含む応答を保存する", async () => {
    mocks.findMessages.mockResolvedValue(
      TRAFFIC_SAFETY_PLAN_QUESTIONS.slice(0, -1).map((question, index) => ({
        id: `user-${index}`,
        role: "user",
        content: `回答${index + 1}`,

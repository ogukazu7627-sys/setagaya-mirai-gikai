...
import { generateInterviewResponse } from "@/features/public-comment/traffic-safety-plan/server/ai";
import {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (previousAnswerCount >= TRAFFIC_SAFETY_PLAN_QUESTIONS.length)
      return NextResponse.json(
    ...
    const isComplete = userAnswerCount >= TRAFFIC_SAFETY_PLAN_QUESTIONS.length;
    const nextQuestion = isComplete
      ? null
      : TRAFFIC_SAFETY_PLAN_QUESTIONS[userAnswerCount];
    ...
    await appendMessage({
      ...
      stage: "interview",
      questionId:
        TRAFFIC_SAFETY_PLAN_QUESTIONS[Math.max(0, userAnswerCount - 1)]?.id ??
        null,
      content,
    });
    ...
    console.error("TrafficSafetyPlan public comment chat error");

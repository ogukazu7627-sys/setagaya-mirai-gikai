...
const request = () =>
  new Request("http://localhost/api/public-comment/ijime/session", {
    method: "POST",
    body: JSON.stringify({
      consented: true,
      receiptOptIn: false,
...
describe("POST /api/public-comment/ijime/session", () => {
  beforeEach(() => {
    vi.resetAllMocks();
    ...
    mocks.findMessages.mockResolvedValue([
      {
        id: "message-1",
        role: "assistant",
        content: IJIME_QUESTIONS[0].question,
        question_id: IJIME_QUESTIONS[0].id,
      },
    ]);
  });
  ...
  it("再開時もメール控えなしの同意を本人のセッションへ保存する", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      sessionId: "session-1",
      nextStage: "interview",
      quickReplies: IJIME_QUESTIONS[0].quickReplies,
    });
  });

  it("最後の回答後に離脱しても下書き作成から再開する", async () => {
    mocks.findMessages.mockResolvedValue(
      IJIME_QUESTIONS.flatMap((question, index) => [
        {
          id: `assistant-${index}`,
          role: "assistant",
  ...
  it("保存済みの下書きがある場合は確認画面から再開する", async () => {

...
import { generateInterviewResponse } from "@/features/public-comment/ijime/server/ai";
import {
  IJIME_CAMPAIGN_SLUG,
  IJIME_QUESTIONS,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (previousAnswerCount >= IJIME_QUESTIONS.length)
      return NextResponse.json(
    ...
    const isComplete = userAnswerCount >= IJIME_QUESTIONS.length;
    const nextQuestion = isComplete ? null : IJIME_QUESTIONS[userAnswerCount];
    ...
    await appendMessage({
      ...
      role: "user",
      stage: "interview",
      questionId: IJIME_QUESTIONS[Math.max(0, userAnswerCount - 1)]?.id ?? null,
      content,
    });
    ...
    console.error("Ijime public comment chat error");

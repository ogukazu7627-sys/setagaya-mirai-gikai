...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (messages.length === 0) {
      const firstQuestion = RETAINING_WALL_QUESTIONS[0];
      await appendMessage({
    ...
    if (answerCount >= RETAINING_WALL_QUESTIONS.length) {
      return NextResponse.json({
        sessionId: session.id,
        messages,
        quickReplies: [],
    ...
    console.error("RetainingWall public comment session error");
    return NextResponse.json(

...
import { generateInterviewResponse } from "@/features/public-comment/retaining-wall/server/ai";
import {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (previousAnswerCount >= RETAINING_WALL_QUESTIONS.length)
      return NextResponse.json(
    ...
    const isComplete = userAnswerCount >= RETAINING_WALL_QUESTIONS.length;
    const nextQuestion = isComplete
      ? null
      : RETAINING_WALL_QUESTIONS[userAnswerCount];
    ...
    await appendMessage({
      ...
      stage: "interview",
      questionId:
        RETAINING_WALL_QUESTIONS[Math.max(0, userAnswerCount - 1)]?.id ?? null,
      content,
    });
    ...
    console.error("RetainingWall public comment chat error");

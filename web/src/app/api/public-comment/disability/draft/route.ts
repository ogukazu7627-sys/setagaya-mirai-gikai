...
export async function POST(request: Request) {
  ...
  try {
    ...
    const draft = await upsertDraft({
      sessionId,
      userId: user.id,
      targetOrdinances: [DISABILITY_ORDINANCE],
      aiBody: output.body,
      finalBody: output.body,
      sourceRefs: DISABILITY_SOURCES.map(({ id, title, url }) => ({
        id,
        title,
        url,
      })),
    ...
    return NextResponse.json({ draft, sources: DISABILITY_SOURCES });
  } catch (error) {
    ...
    console.error("Disability public comment draft generation error");
    return NextResponse.json(
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    console.error("Disability public comment draft update error");
    return NextResponse.json(

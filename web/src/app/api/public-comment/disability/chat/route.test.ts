...
const request = () =>
  new Request("http://localhost/api/public-comment/disability/chat", {
    method: "POST",
    body: JSON.stringify({ sessionId: "session-1", content: "私の意見" }),
  });

describe("POST /api/public-comment/disability/chat", () => {
  beforeEach(() => {
    vi.resetAllMocks();
    mocks.getUser.mockResolvedValue({ id: "owner" });
    mocks.findCampaign.mockResolvedValue({ id: "campaign-disability" });
  ...
  it("最終回答もAIを通し、安全案内を含む応答を保存する", async () => {
    mocks.findMessages.mockResolvedValue(
      DISABILITY_QUESTIONS.slice(0, -1).map((question, index) => ({
        id: `user-${index}`,
        role: "user",
        content: `回答${index + 1}`,

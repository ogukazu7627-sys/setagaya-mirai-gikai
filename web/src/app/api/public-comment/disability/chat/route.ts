...
import { generateInterviewResponse } from "@/features/public-comment/disability/server/ai";
import {
...
export async function POST(request: Request) {
  ...
  try {
    const campaign = await findCampaign(DISABILITY_CAMPAIGN_SLUG);
    if (!campaign)
      return NextResponse.json(
    ...
    if (previousAnswerCount >= DISABILITY_QUESTIONS.length)
      return NextResponse.json(
    ...
    const isComplete = userAnswerCount >= DISABILITY_QUESTIONS.length;
    const nextQuestion = isComplete
      ? null
      : DISABILITY_QUESTIONS[userAnswerCount];
    ...
    await appendMessage({
      ...
      stage: "interview",
      questionId:
        DISABILITY_QUESTIONS[Math.max(0, userAnswerCount - 1)]?.id ?? null,
      content,
    });
    ...
    console.error("Disability public comment chat error");

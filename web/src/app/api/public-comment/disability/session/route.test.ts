...
import { DISABILITY_QUESTIONS } from "@/features/public-comment/disability/shared/campaign";
...
const request = () =>
  new Request("http://localhost/api/public-comment/disability/session", {
    method: "POST",
    body: JSON.stringify({
      consented: true,
      receiptOptIn: false,
...
describe("POST /api/public-comment/disability/session", () => {
    ...
    mocks.findMessages.mockResolvedValue([
      {
        id: "message-1",
        role: "assistant",
        content: DISABILITY_QUESTIONS[0].question,
        question_id: DISABILITY_QUESTIONS[0].id,
      },
    ]);
  });
  ...
  it("再開時もメール控えなしの同意を本人のセッションへ保存する", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      sessionId: "session-1",
      nextStage: "interview",
      quickReplies: DISABILITY_QUESTIONS[0].quickReplies,
    });
  });

  it("7問の回答後は下書き作成から再開する", async () => {
    mocks.findMessages.mockResolvedValue(
      DISABILITY_QUESTIONS.flatMap((question, index) => [
        {
          id: `assistant-${index}`,
          role: "assistant",

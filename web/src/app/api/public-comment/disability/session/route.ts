...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (messages.length === 0) {
      const firstQuestion = DISABILITY_QUESTIONS[0];
      await appendMessage({
    ...
    if (answerCount >= DISABILITY_QUESTIONS.length) {
      return NextResponse.json({
        sessionId: session.id,
        messages,
        quickReplies: [],
    ...
    const currentQuestion =
      DISABILITY_QUESTIONS.find(
        (question) => question.id === lastAssistantQuestionId
      ) ?? DISABILITY_QUESTIONS[0];
    ...
    console.error("Disability public comment session error");
    return NextResponse.json(

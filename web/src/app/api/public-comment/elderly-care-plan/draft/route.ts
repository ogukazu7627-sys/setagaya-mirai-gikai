...
export async function POST(request: Request) {
  ...
  try {
    ...
    const draft = await upsertDraft({
      sessionId,
      userId: user.id,
      targetOrdinances: [ELDERLY_CARE_PLAN_PLAN],
      aiBody: output.body,
      finalBody: output.body,
      sourceRefs: ELDERLY_CARE_PLAN_SOURCES.map(({ id, title, url }) => ({
        id,
        title,
        url,
      })),
    ...
    return NextResponse.json({ draft, sources: ELDERLY_CARE_PLAN_SOURCES });
  } catch (error) {
    ...
    console.error("ElderlyCarePlan public comment draft generation error");
    return NextResponse.json(
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    console.error("ElderlyCarePlan public comment draft update error");
    return NextResponse.json(

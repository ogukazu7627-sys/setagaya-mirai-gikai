...
import { generateInterviewResponse } from "@/features/public-comment/elderly-care-plan/server/ai";
import {
...
export async function POST(request: Request) {
  ...
  try {
    const campaign = await findCampaign(ELDERLY_CARE_PLAN_CAMPAIGN_SLUG);
    if (!campaign)
      return NextResponse.json(
    ...
    if (previousAnswerCount >= ELDERLY_CARE_PLAN_QUESTIONS.length)
      return NextResponse.json(
    ...
    const isComplete = userAnswerCount >= ELDERLY_CARE_PLAN_QUESTIONS.length;
    const nextQuestion = isComplete
      ? null
      : ELDERLY_CARE_PLAN_QUESTIONS[userAnswerCount];
    ...
    await appendMessage({
      ...
      stage: "interview",
      questionId:
        ELDERLY_CARE_PLAN_QUESTIONS[Math.max(0, userAnswerCount - 1)]?.id ??
        null,
      content,
    });
    ...
    console.error("ElderlyCarePlan public comment chat error");

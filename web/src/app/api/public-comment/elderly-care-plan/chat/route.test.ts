...
const request = () =>
  new Request("http://localhost/api/public-comment/elderly-care-plan/chat", {
    method: "POST",
    body: JSON.stringify({ sessionId: "session-1", content: "私の意見" }),
  });

describe("POST /api/public-comment/elderly-care-plan/chat", () => {
  beforeEach(() => {
    vi.resetAllMocks();
    mocks.getUser.mockResolvedValue({ id: "owner" });
    mocks.findCampaign.mockResolvedValue({ id: "campaign-elderly-care-plan" });
  ...
  it("最終回答もAIを通し、安全案内を含む応答を保存する", async () => {
    mocks.findMessages.mockResolvedValue(
      ELDERLY_CARE_PLAN_QUESTIONS.slice(0, -1).map((question, index) => ({
        id: `user-${index}`,
        role: "user",
        content: `回答${index + 1}`,

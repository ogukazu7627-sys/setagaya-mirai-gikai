...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (messages.length === 0) {
      const firstQuestion = ELDERLY_CARE_PLAN_QUESTIONS[0];
      await appendMessage({
    ...
    if (answerCount >= ELDERLY_CARE_PLAN_QUESTIONS.length) {
      return NextResponse.json({
        sessionId: session.id,
        messages,
        quickReplies: [],
    ...
    console.error("ElderlyCarePlan public comment session error");
    return NextResponse.json(

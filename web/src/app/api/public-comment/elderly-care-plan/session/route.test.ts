...
import { ELDERLY_CARE_PLAN_QUESTIONS } from "@/features/public-comment/elderly-care-plan/shared/campaign";
...
const request = () =>
  new Request("http://localhost/api/public-comment/elderly-care-plan/session", {
    method: "POST",
    body: JSON.stringify({
      consented: true,
      receiptOptIn: false,
...
describe("POST /api/public-comment/elderly-care-plan/session", () => {
  beforeEach(() => {
    vi.resetAllMocks();
    mocks.getUser.mockResolvedValue({ id: "google-user" });
    mocks.findCampaign.mockResolvedValue({
      id: "campaign-elderly-care-plan",
      status: "published",
    });
    ...
    mocks.findMessages.mockResolvedValue([
      {
        id: "message-1",
        role: "assistant",
        content: ELDERLY_CARE_PLAN_QUESTIONS[0].question,
        question_id: ELDERLY_CARE_PLAN_QUESTIONS[0].id,
      },
    ]);
  });
  ...
  it("再開時もメール控えなしの同意を本人のセッションへ保存する", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      sessionId: "session-1",
      nextStage: "interview",
      quickReplies: ELDERLY_CARE_PLAN_QUESTIONS[0].quickReplies,
    });
  });

  it("7問の回答後は下書き作成から再開する", async () => {
    mocks.findMessages.mockResolvedValue(
      ELDERLY_CARE_PLAN_QUESTIONS.flatMap((question, index) => [
        {
          id: `assistant-${index}`,
          role: "assistant",

  ...
  try {
    ...
    if (
      !canCreateInterviewDraft(
        ...
        SUICIDE_PREVENTION_QUESTIONS.length
      )
    )
      return NextResponse.json(
    ...
    const draft = await upsertDraft({
      sessionId,
      userId: user.id,
      targetOrdinances: [SUICIDE_PREVENTION_PLAN],
      aiBody: output.body,
      finalBody: output.body,
      sourceRefs: SUICIDE_PREVENTION_SOURCES.map(({ id, title, url }) => ({
        id,
        title,
        url,
      })),
    ...
    return NextResponse.json({ draft, sources: SUICIDE_PREVENTION_SOURCES });
  } catch (error) {
    ...
    console.error("SuicidePrevention public comment draft generation error");
    return NextResponse.json(
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    console.error("SuicidePrevention public comment draft update error");
    return NextResponse.json(

...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (messages.length === 0) {
      const firstQuestion = SUICIDE_PREVENTION_QUESTIONS[0];
      await appendMessage({
    ...
    if (answerCount >= SUICIDE_PREVENTION_QUESTIONS.length) {
      return NextResponse.json({
        sessionId: session.id,
        messages,
        quickReplies: [],
    ...
    const currentQuestion =
      ...
      SUICIDE_PREVENTION_QUESTIONS[answerCount] ??
      SUICIDE_PREVENTION_QUESTIONS[0];
    ...
    console.error("SuicidePrevention public comment session error");
    return NextResponse.json(

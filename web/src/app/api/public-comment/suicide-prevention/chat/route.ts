...
import { generateInterviewResponse } from "@/features/public-comment/suicide-prevention/server/ai";
import {
...
export async function POST(request: Request) {
  ...
  try {
    const campaign = await findCampaign(SUICIDE_PREVENTION_CAMPAIGN_SLUG);
    if (!campaign)
      return NextResponse.json(
    ...
    if (previousAnswerCount >= SUICIDE_PREVENTION_QUESTIONS.length)
      return NextResponse.json(
    ...
    const isComplete = userAnswerCount >= SUICIDE_PREVENTION_QUESTIONS.length;
    const nextQuestion = isComplete
      ? null
      : SUICIDE_PREVENTION_QUESTIONS[userAnswerCount];
    ...
    const safetyRedirect =
      response.next_stage === "interview" &&
      response.question_id === null &&
      response.topic_title === null;
    ...
    if (!safetyRedirect) {
      await appendMessage({
        ...
        stage: "interview",
        questionId:
          SUICIDE_PREVENTION_QUESTIONS[Math.max(0, userAnswerCount - 1)]?.id ??
          null,
        content,
      });
    }
    const assistantMessage = await appendMessage({
      ...
      role: "assistant",
      stage: "interview",
      questionId: safetyRedirect ? null : (nextQuestion?.id ?? null),
      content: response.text,
    });
    return NextResponse.json({
      message: assistantMessage,
      userMessageStored: !safetyRedirect,
      nextStage: safetyRedirect
        ? "interview"
        : isComplete
          ? "draft"
          : "interview",
      quickReplies: safetyRedirect
        ? []
        : !isComplete && response.quick_replies.length > 0
          ? response.quick_replies
          : (nextQuestion?.quickReplies ?? []),
      topicTitle: safetyRedirect
        ? null
        : (response.topic_title ?? nextQuestion?.topic ?? null),
    });
  } catch (error) {
    ...
    console.error("SuicidePrevention public comment chat error");

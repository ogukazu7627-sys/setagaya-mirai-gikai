  ...
  return (
    ...
    >
      <Container className="space-y-10">
        ...
        <LegalParagraph>
          みらい議会＠世田谷区（以下「本サービス」といいます。）をご利用いただく場合、以下の規約に同意いただいたものとみなします。本サービスは、世田谷区議会の公開資料をわかりやすく確認するための情報整理サイトであり、世田谷区公認のサイトではありません。
        </LegalParagraph>
        ...
        <section className="space-y-4">
          ...
          <LegalParagraph>
            パブリックコメントをつくるAIインタビューは、同意後に一時的な匿名IDを発行し、Googleログインなしで開始できます。会話と作成中の下書きはこの匿名IDにひも付けて保存します。最終文章の表示と編集には、不正利用・過剰利用の防止と利用上限の管理のためGoogleログインが必要です。ログイン後は、匿名IDに保存した今回の会話と下書きを、確認済みのGoogleアカウントに引き継ぎます。
          </LegalParagraph>
          <LegalParagraph>{PUBLIC_COMMENT_EMAIL_NOTICE}</LegalParagraph>
          <LegalParagraph>
            控えメールとイベント案内メールの受信欄は初期状態でチェックされていますが、利用規約・回答保存への必須同意とは別の任意設定です。最終文章を表示するためのGoogleログイン画面と下書き確認画面でそれぞれ変更でき、選択・日時・同意文面の版を記録します。送信にはResendを使用します。イベント案内の受信は、控えの受信、コメントの匿名公開、議員への連絡先共有への同意とは別です。イベント案内にはインタビューの個別回答を含めません。メールは区への正式な提出を行うものではなく、到着を保証するものでもありません。
          </LegalParagraph>
        </section>
      </Container>
    </LegalPageLayout>

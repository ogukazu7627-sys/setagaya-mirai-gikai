...
type GeneralQuestionCategoryRouteProps = {
  params: Promise<{ categoryId: string; sessionKey: string; year: string }>;
...
export async function generateMetadata({
  ...
  const { categoryId, sessionKey, year: yearParam } = await params;
  ...
  const description = `${year}年の世田谷区議会で行われた${category.name}分野の一般質問を、定例会ごと・議員ごとに確認できます。`;
...
export default async function GeneralQuestionCategoryRoutePage({
  ...
  const [routeParams, { focus }, difficultyLevel] = await Promise.all([
    params,
    searchParams,
    getDifficultyLevel(),
  ]);
  const { categoryId, sessionKey, year: yearParam } = routeParams;
  const year = parseCalendarYear(yearParam);

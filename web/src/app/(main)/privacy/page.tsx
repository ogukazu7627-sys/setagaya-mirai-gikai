    ...
    >
      <Container className="space-y-8">
        ...
        <section className="space-y-4">
          ...
          <LegalParagraph>
            控えとイベント案内の受信設定、選択日時、同意文面の版をインタビューごとに保存します。送信のため、Googleログインで取得したメールアドレス、控えの場合は会話全文と本人確認済みコメント、イベント案内の場合は固定の案内文をメール配信サービスResendへ渡します。重複送信の防止と障害確認のため、送信先・確定した本文・HTML本文・送信状態も保存します。イベント案内に個別のインタビュー回答を含めません。メールアドレスや受信設定をAIへの入力や公開コメントには含めません。開封・クリックの追跡は行いません。
          </LegalParagraph>
          <LegalParagraph>
            控えまたはイベント案内が不要な場合は、最終文章を表示するためのGoogleログイン画面または下書き確認画面で該当する受信欄のチェックを外してください。完了後に送信済みのメールは取り消せません。以前取得した活動案内の配信希望は別の記録として保持し、今回のメール送信には使用しません。イベント案内の配信停止や、個人情報の確認・削除は本サービスのお問い合わせ窓口へご相談ください。
          </LegalParagraph>
        </section>
